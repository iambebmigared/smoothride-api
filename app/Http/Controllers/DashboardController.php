...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $trip = Trip::where('tripRequest','approved')->count('id');
        $business = User::where('userType','business')->count('id');
        $driver = User::where('userType','driver')->count('id');

        $totaltripAmt = Trip::where('tripAmt', '!=', null)->where(['tripRequest' => 'approved'])->sum('tripAmt');
        $totaltripAmtDay = Trip::where('tripAmt', '!=', null)->whereYear('created_at',date("Y"))->whereMonth('created_at',date("m"))->whereDay('created_at', date("d"))->where(['tripRequest' => 'approved'])->sum('tripAmt');
        $totaltripAmtMonth = Trip::where('tripAmt', '!=', null)->whereYear('created_at',date("Y"))->whereMonth('created_at',date("m"))->where(['tripRequest' => 'approved'])->sum('tripAmt');
        $totaltripAmtMonthPrev = Trip::where('tripAmt', '!=', null)->whereYear('created_at',date("Y"))->whereMonth('created_at',date("m")-1)->where([ 'tripRequest' => 'approved'])->sum('tripAmt');
        $totaltripAmtYear = Trip::where('tripAmt', '!=', null)->whereYear('created_at',date("Y"))->where(['tripRequest' => 'approved'])->sum('tripAmt');
        $totaltripAmtWeek = Trip::where('tripAmt', '!=', null)->whereYear('created_at',date("Y"))->whereMonth('created_at',date("m"))->where(\DB::raw("WEEKOFYEAR(created_at)"), $dt->weekOfYear)->where('tripRequest','approved')->sum('tripAmt');
