...
//        validation
 ...
 function tripsWithBranchFLimited($unique_code){
    ...
    //This is the function, can you work on this for permanent solution
    // the page crash on loading data and also i was not able to download report via excel
  ...
  /*  
   ...
   /* 
    ...
    {  
       ...
       //$alltrips = collect();
        $alltrips = array();
        //$users=  User::where(['userType' => 'branch','business_code'=> Auth::user()->unique_code ])->paginate(5);
       $users=  User::where(['userType' => 'branch','business_code'=> Auth::user()->unique_code ])->orderBy('id', 'asc')->get();
        //dd($users);
        //return count($users);
    // $alltrips->push($this->tripsWithBranchHQ());
        //$alltrips = [];
        foreach($users as $key => $name)
         {
             ...
             //$alltrips[] = $this->tripsWithBranchF($name->unique_code);
            // print_r($this->tripsWithBranchF($name->unique_code));
           // if(count($this->tripsWithBranchF($name->unique_code)>0))
         // array_push($alltrips,...$this->tripsWithBranchF($name->unique_code));
          $variables = $this->tripsWithBranchF($name->unique_code);
       ...
       //  dd($name->unique_code);
         ...
         $values = $this->tripsWithBranchF($name->unique_code);
            foreach ($values as $va)
            {
                $alltrips[] = $va;
            }
        
        //$alltrips[] = $this->tripsWithBranchHQ();
  ...
  // dd($alltrips);
    ...
    */
    ...
    public function tripwithbranchfilter(Request $request)
    {
     ...
     public function searchBranchBusinessTrip(Request $request)
    {
      ...
      $from = $request->first ?? Carbon::now();
       ...
       $users=  User::where(['userType' => 'branch','business_code'=> Auth::user()->unique_code ])->get();
        ...
        $parent_trips = Trip::whereBetween('', [$from, $to])->where(['parent_code' => Auth::user()->unique_code, 'tripRequest' => 'approved', 'flag' => 0]);
