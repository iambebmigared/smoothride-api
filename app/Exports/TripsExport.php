...
class TripsExport implements FromQuery, WithHeadings, WithMapping
{
    ...
    {   
        ...
        if(isset($trip->tripEndTime)){
            $start_time = \Carbon\Carbon::parse($trip->tripEndTime);
        }
        
        if(isset($trip->tripEndTime) && isset($trip->travelTime)){
            $start_time = \Carbon\Carbon::parse($trip->tripEndTime)->addSeconds($trip->travelTime);
        }
